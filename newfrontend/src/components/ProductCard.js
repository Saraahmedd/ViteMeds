...
const { BottomCallout } = require("./BottomCallout");
const { useSelect } = require("@material-tailwind/react");
...
function ProductCard({
  ...
  const { error: addError, loading: addLoading } = useSelector(
    (state) => state.addToCartReducer
  );

  const [prescErr, setPrescError] = useState(false);
  ...
  useEffect(() => {
    ...
    // Clear the timeout if the component unmounts or if stkError becomes false before the timeout
  ...
  return (
    <>
      ...
      {prescErr && (
        <BottomCallout
          message="This medicine requires a prescription and you don't have one"
          visible={stkError}
          setVisible={setStkError}
        />
      )}
