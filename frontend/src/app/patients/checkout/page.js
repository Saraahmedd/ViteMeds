                    ...
                    <tbody>
                      {patient?.deliveryAddress?.map((address) => (
                        <tr key={address._id}>
                          ...
                          <td>
                             <button onClick={() => onNext(address._id)} style={buttonStyle}>Next</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </Table>
...
const Checkout = () => {
  ...
  return (
    ...
    gap: '20px', }}>
          <Stepper currentStep={currentStep} totalSteps={3} onPrev={() => setCurrentStep((prev) => Math.max(prev - 1, 1))} onNext={() => setCurrentStep((prev) => Math.min(prev + 1, totalSteps))} />
      {currentStep === 1 && (
        <ShippingInformation    onPrev={() => setCurrentStep(currentStep - 1)} patient={patient} onNext={(data) => {
