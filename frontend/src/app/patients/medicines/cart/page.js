"use client"
import { addToCart, deleteFromCart, viewCart } from "@/app/redux/actions/cartActions";
...
export default function Cart() {
    ...
    const addToCartSel = useSelector(
        (state) => state.addToCartReducer.cart
    );
    ...
    return (
        <>
            <ul className="list-group">
                {
                    cart && cart.cart.items.map((item, index) => {
                        return (
                            <li key={item._id} className="list-group-item">
                                <div className="d-flex flex-row align-items-center p-2">
                                    <img style={{ maxHeight: '100px' }} className="me-2" src={`http://localhost:8000/${item.medicine.imageURL}`} />
                                    <h3>{item.medicine.name} ({item.quantity})</h3>
           ...
           {cart && <Button onClick={() => window.location.href ='/patients/checkout'}>Checkout</Button>}
        </>
    )
}