...
export default function DoctorApps() {
  ...
  const doctors = useSelector(
    (state) => state.getPharmacistsReducer.pharmacists,
  );
  const isLoading = useSelector((state) => state.removeUserReducer.loading);
  const approvalisLoading = useSelector(
    (state) => state.adminAcceptPharmacistReducer.loading,
  );
  useEffect(() => {
      ...
      <div className="justify-content-center align-items-center min-vh-100 mx-auto ">
        <div className="row">
          {doctors?.data?.map((person) => {
            if (person.isApproved) return;
            return (
              <div className="mx-auto col-md-6">
                <Card
                  key={person.username}
                  className="col-lg-8 mx-auto offset-lg-1 my-4 bg-light"
                  title={
                    <div className="text-capitalize ps-3">{person.name}</div>
                  }
                >
                  {/* {button} */}
                  <div className="p-3">
                    ...
                    <div className="row global-text mb-1">
                      <div className="col-md-6">
                        <h8 style={{ fontWeight: "bold" }}>Affiliation: </h8>
                        {person.affiliation}
                        <br />
                      </div>
                      <div className="col-md-6">
                        <h8 style={{ fontWeight: "bold" }}>Hourly Rate: </h8>
                        {person.hourlyRate}
                        <br />
                      </div>
                    </div>
